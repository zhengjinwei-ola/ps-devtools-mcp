...
func TestQueryTestDBToolCanBeCalledOverMCP(t *testing.T) {
	...
	serverSession, err := New(Services{DB: service, Redis: &stubRedisService{}}).Connect(ctx, serverTransport, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestQueryTestRedisToolCanBeCalledOverMCP(t *testing.T) {
	...
	serverTransport, clientTransport := mcp.NewInMemoryTransports()
	serverSession, err := New(Services{DB: &stubService{}, Redis: redisService}).Connect(ctx, serverTransport, nil)
	if err != nil {
		t.Fatal(err)
	}
	defer serverSession.Close()
